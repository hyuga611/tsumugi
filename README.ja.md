...
**同じ版なら何も取ってきません**（`すでに最新です` と言って終わります）。

`tsg update` は**自分で取ってきます** — 外のプロセスは 1 つも起こしません。
前は `powershell -Command "irm <URL> | iex"` を起こしていましたが、
**その形は「取ってきて、その場で実行する」マルウェアと見分けが付かず**、
会社の PC の Windows Defender に止められました（起動を拒否されたうえに
置いてあった exe まで消えました）。要る場所ほど厳しいので、まさにそこで
壊れる作りでした。

走っている自分は消せないので、前の版は `tsg.old-…` として残り、次の
`tsg update` で消えます。`cargo build` で作った exe には**上書きしません**
（断って、代わりの手を出します）。
...
| 下の `● 返事待ち N` | 押すとそこへ飛ぶ |
| `Space a` | 次の返事待ちへ |
| そのペインに打つ | 印が下ります（応えたので）。状態そのものは残るので `--agents` には出たままで、エージェントが次に名乗ればまた点きます |

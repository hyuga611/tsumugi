...
/// サーバを止める。**Attach を通さない。**
///
/// ここが `attach()` を通ると、版の違うサーバを落とせなくなる。
/// 版が違えばサーバは Attach を弾く（`tsg-mux` の版チェック）ので、
/// 便利な口はどれも「繋げるが何も言えない」状態になり、プロトコルを
/// 上げるたびに**古いサーバを PID で探して殺す**しか無くなる。
/// 実際に起きた: 旧ビルドのサーバが残ったまま新しい exe で開くと、
/// 窓には版違いだけが出て、`--rpc` からの `shutdown` も届かなかった。
///
/// `Shutdown` は Attach の前でも受ける約束にしてあるので、ここは
/// 繋いで一言だけ送る。**この口だけは版に依らないこと**が値打ちなので、
/// ついでの用事（形を保存する等）をここに足さない。
pub fn kill(session: &str) -> Result<()> {
    let mut client = Client::connect(session)
        .with_context(|| format!("セッション '{session}' は走っていません"))?;
    client.send(&ClientMsg::Shutdown)?;
    // 送っただけでは「止めた」と言えない。繋がらなくなるまで見届ける。
    // ここを省くと、止まっていないのに止めたと言い、次に開いた窓が
    // 同じ版違いに当たる。
    for _ in 0..100 {
        std::thread::sleep(Duration::from_millis(50));
        if Client::connect(session).is_err() {
            eprintln!("セッション '{session}' を止めました");
            return Ok(());
        }
    }
    bail!("セッション '{session}' が 5 秒以内に止まりませんでした")
}
